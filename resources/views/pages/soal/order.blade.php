...
@section('content')
    <div class="container-fluid px-4" id="soal-order-container">
        <div class="row">
            <div class="col-12">
                <div class="bg-white rounded-4 shadow-sm p-8 mb-5">
                    <form action="" method="POST" id="form-level-order" onsubmit="updateOrderSoal(); return false;">
                        ...
                        <div class="row align-items-start mb-5">
                            <div class="col-md-8">
                                <h4 class="mb-2">Urutkan Soal</h4>
                                <p class="text-muted mb-0">Drag and drop untuk mengurutkan Soal sesuai keinginan Anda.</p>
                            </div>
                            <div class="col-md-4">
                        ...
                        <div class="row draggable-zone" id="sortable-soal-list">
                        </div>
...
@push('scripts')
    ...
    <script>
        var soalSortable = null;

        function initSortableIfNeeded() {
            const el = document.getElementById('sortable-soal-list');
            if (!el) return;
            if (soalSortable) {
                // optional: update if needed
                return;
            }
            soalSortable = new Sortable(el, {
                animation: 120,
                draggable: ".draggable",
        ...
        $('#level_select').on('change', function() {
            ...
            const $list = $('#sortable-soal-list');
            $list.empty();
            soalSortable = null; // reset agar fresh untuk level baru
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (!Array.isArray(response) || response.length === 0) {
                        $list.html('<div class="col-12"><div class="alert alert-warning mb-0">Tidak ada soal.</div></div>');
                        return;
                    }
                ...
                error: function(xhr) {
                    Swal.fire({
                        text: xhr.responseJSON?.message || "Terjadi kesalahan saat memuat soal untuk level yang dipilih.",
                        icon: "error",
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response.success) {
                                ...
                                Swal.fire({
                                    text: response.message || "Gagal menyimpan urutan Soal.",
                                    icon: "error",
                                    buttonsStyling: false,
                                    confirmButtonText: "OK",
                                    customClass: {
