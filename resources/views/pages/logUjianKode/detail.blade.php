...
@push('scripts')
    @php
        $logUjianKodeDetailScripts = [
            public_path('js/logujiankode/detail.js'),
            public_path('js/logUjianKode/detail.js'),
        ];
        $logUjianKodeDetailScript = collect($logUjianKodeDetailScripts)->first(fn ($path) => is_file($path));
    @endphp

    @if ($logUjianKodeDetailScript)
        <script>
