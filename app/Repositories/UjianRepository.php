...
class UjianRepository extends BaseRepository
{
    ...
    public function submit($request)
    {
        try {
            ...
            if (count($jawabanTipe) !== count($kunciTipe)) {
                $isCorrectTipe = false;
                $tipeMismatch[] = [
                    'reason'         => 'length_not_match',
                    'expected_count' => count($kunciTipe),
                    'given_count'    => count($jawabanTipe),
                ];
            } else {
            ...
            if (count($jawabLangkah) !== count($kunciLangkah)) {
                ...
                $algoMismatch[] = [
                    'reason'         => 'length_not_match',
                    'expected_count' => count($kunciLangkah),
                    'given_count'    => count($jawabLangkah),
                ];
            } else {
            ...
            if ($dataPencapaian && $dataPencapaian->status == 0 && $isCorrectAll) {
                ...
                $returnPencapaian = ['id' => $dataPencapaian->id];
            }
            ...
            if ($isCorrectAll) {
                ...
                if ($label === 'Ideal' || $label === 'Normal') {
                    ...
                    if ($dataPencapaianBadge && $dataPencapaianBadge->status == 0 && $isCorrectAll) {
                        ...
                        $returnPencapaianBadge = ['id' => $dataPencapaianBadge->id];
                    }
                }
    ...
    private function determineLabelAndScore($totalDrag, $totalWaktuDetik): array
    {
    ...
    private function syncAdaptiveRealtimeLogOnCorrectSubmit(
        string $idMahasiswa,
        string $idSoal,
        int $totalWaktuDetik,
        bool $isCorrect = true
